 ...
 *
 * @author Liuyingjie
 * @datetime 2022/9/1 23:18
 */
@Data
@ApiModel(description = "修改萌宠播报任务请求VO")
public class UpdateBroadcastTaskReqVO {
    ...
    @ApiModelProperty(value = "执行策略；AVNT_ONLINE(车辆上电)、USER_LOGIN(账号登陆)",
            allowableValues = "AVNT_ONLINE, USER_LOGIN", required = true, position = 4)
    ...
    @ApiModelProperty(value = "执行频率，支持类型：ONE_TIME:(推送一次)、EVERY_DAY:(每天一次)",
            allowableValues = "ONE_TIME, EVERY_DAY", required = true, position = 5)
    ...
    @ApiModelProperty(value = "发送范围：SINGLE(单个VIN)、MULTIPLE(批量VIN)、COMBINATION(组合条件))",
            allowableValues = "SINGLE, MULTIPLE, COMBINATION", required = true, position = 6)
    @Pattern(regexp = "SINGLE|MULTIPLE|COMBINATION", message = "发送范围类型不符")
    private String scope;

    @ApiModelProperty(value = "节假日主键ID", required = true, position = 7)
    private Long holidayId;

    @ApiModelProperty(value = "车型ID列表", position = 8)
    private List<Long> mtIds;

    @ApiModelProperty(value = "批量车辆VIN.xlsx文件(批量VIN时填写)", position = 9)
